...
use App\Http\Controllers\AccountingSalesLedgerController;
use App\Http\Controllers\AccountingWorkflowSimulationController;
use App\Http\Controllers\Admin\OrderActivityController;
use App\Http\Controllers\Admin\OrderFeeTypeController;
...
use App\Http\Controllers\Admin\TruckBrandController;
use App\Http\Controllers\Admin\TruckRouteController;
use App\Http\Controllers\Admin\TruckStationAdminController;
use App\Http\Controllers\AdminDailyRebuildController;
use App\Http\Controllers\AdminEventController;
use App\Http\Controllers\AdminNotificationController;
use App\Http\Controllers\AdminOrderDeletionController;
...
use App\Http\Controllers\CeoChangePasswordController;
use App\Http\Controllers\CeoDashboardController;
use App\Http\Controllers\CeoPageController;
...
use App\Http\Controllers\GoogleSheetsOrderReviewController;
use App\Http\Controllers\HoangLongProfileController;
use App\Http\Controllers\HomeController;
use App\Http\Controllers\ImportedSalesOrderCompletionController;
use App\Http\Controllers\InventoryAdjustmentController;
use App\Http\Controllers\InventoryController;
use App\Http\Controllers\InventoryDocumentController;
use App\Http\Controllers\InventoryDocumentTemplateController;
...
use App\Http\Controllers\ProcurementPurchaseTemplateController;
use App\Http\Controllers\ProductController;
use App\Http\Controllers\ProductPriceManagementController;
use App\Http\Controllers\ProductSpreadsheetController;
...
use App\Http\Controllers\TaskDelegateConfigController;
...
use App\Http\Controllers\Warehouse\OrderShippingLabelController;
use App\Http\Controllers\Warehouse\OrderTransferController;
use App\Http\Controllers\Warehouse\WarehouseDispatchSlipController;
use App\Http\Controllers\WarehouseController;
use App\Http\Controllers\WarehouseDashboardController;
use App\Http\Controllers\WarehouseGoogleSheetInventoryController;
use App\Http\Controllers\WarehouseProcurementController;
use App\Http\Controllers\WarehouseStocktakeController;
use App\Http\Controllers\WarehouseSupplierPriceController;
...
Route::middleware(['auth', 'assigned'])->group(function () {
    ...
    Route::post('/my-customer/refresh-priority', [MyCustomerController::class, 'refreshPriority'])->name('my_customer.refresh_priority');
    ...
    // Customer Reminders
    Route::post('/my-customer/{customer}/reminders', [CustomerReminderController::class, 'store'])->name('customer_reminders.store');
    Route::put('/my-customer/{customer}/reminders/{reminder}', [CustomerReminderController::class, 'update'])->name('customer_reminders.update');
    Route::delete('/my-customer/{customer}/reminders/{reminder}', [CustomerReminderController::class, 'destroy'])->name('customer_reminders.destroy');
    ...
    Route::get('customer-tracking', [CustomerTrackingController::class, 'index'])
        ->name('customer-tracking.index')
        ->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::get('customer-tracking/data', [CustomerTrackingController::class, 'data'])
        ->name('customer-tracking.data')
        ->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::get('customer-tracking/{customer}', [CustomerTrackingController::class, 'show'])
        ->name('customer-tracking.show')
        ->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::get('customer-tracking/{customer}/data', [CustomerTrackingController::class, 'customerData'])
    ...
    Route::get('/api/suppliers/{supplier}/products', [WarehouseSupplierPriceController::class, 'supplierProducts'])->name('api.suppliers.products');
    Route::get('/api/suppliers/{supplier}/products/{product}/latest-price', [WarehouseSupplierPriceController::class, 'latestPrice'])->name('api.suppliers.products.latest-price');
    Route::get('/api/suppliers/{supplier}/products/{product}/price-history', [WarehouseSupplierPriceController::class, 'priceHistory'])->name('api.suppliers.products.price-history');
    Route::post('/api/supplier-prices', [WarehouseSupplierPriceController::class, 'store'])->name('api.supplier-prices.store');
    ...
    Route::middleware('role:admin,warehouse,account,accountant,accounting')->group(function () {
        Route::get('/admin/imported-sales-orders', [ImportedSalesOrderCompletionController::class, 'index'])->name('admin.imported-sales-orders.index');
        Route::put('/admin/imported-sales-orders/{order}', [ImportedSalesOrderCompletionController::class, 'update'])->name('admin.imported-sales-orders.update');
    });
    ...
    Route::prefix('manager')->name('manager.')->middleware('role:manager,manager_sale,admin')->group(function () {
        ...
        Route::get('/requests/{transaction}/edit', [DepartmentFinanceRequestController::class, 'managerEdit'])->name('finance-requests.edit');
        Route::put('/requests/{transaction}', [DepartmentFinanceRequestController::class, 'managerUpdate'])->name('finance-requests.update');
        Route::delete('/requests/{transaction}', [DepartmentFinanceRequestController::class, 'managerDestroy'])->name('finance-requests.destroy');
    ...
    Route::prefix('accounting')->name('accounting.')->middleware('role:account,accountant,accounting,admin')->group(function () {
        Route::get('/workflow-simulation', [AccountingWorkflowSimulationController::class, 'index'])->name('workflow-simulation.index');
        Route::post('/workflow-simulation/stock-in', [AccountingWorkflowSimulationController::class, 'stockIn'])->name('workflow-simulation.stock-in');
        Route::post('/workflow-simulation/stocktake', [AccountingWorkflowSimulationController::class, 'stocktake'])->name('workflow-simulation.stocktake');
        Route::post('/workflow-simulation/orders', [AccountingWorkflowSimulationController::class, 'createOrder'])->name('workflow-simulation.orders.create');
        Route::post('/workflow-simulation/orders/bulk', [AccountingWorkflowSimulationController::class, 'bulkOrders'])->name('workflow-simulation.orders.bulk');
        Route::post('/workflow-simulation/orders/advance', [AccountingWorkflowSimulationController::class, 'advanceOrders'])->name('workflow-simulation.orders.advance');
        Route::put('/workflow-simulation/orders/{order}/adjust-stock', [AccountingWorkflowSimulationController::class, 'adjustOrderToStock'])->name('workflow-simulation.orders.adjust-stock');
        Route::post('/workflow-simulation/transfers', [AccountingWorkflowSimulationController::class, 'createTransfers'])->name('workflow-simulation.transfers.create');
        Route::post('/workflow-simulation/transfers/advance', [AccountingWorkflowSimulationController::class, 'advanceTransfers'])->name('workflow-simulation.transfers.advance');
        Route::post('/workflow-simulation/transfers/receive-all', [AccountingWorkflowSimulationController::class, 'receiveAll'])->name('workflow-simulation.transfers.receive-all');
        Route::post('/workflow-simulation/assign-orders', [AccountingWorkflowSimulationController::class, 'assignOrders'])->name('workflow-simulation.assign-orders');
        Route::post('/workflow-simulation/deliver-orders', [AccountingWorkflowSimulationController::class, 'deliverOrders'])->name('workflow-simulation.deliver-orders');
        Route::post('/workflow-simulation/confirm-orders', [AccountingWorkflowSimulationController::class, 'confirmOrders'])->name('workflow-simulation.confirm-orders');
        Route::get('/sales-ledger/export', [AccountingSalesLedgerController::class, 'export'])->name('sales-ledger.export');
        Route::post('/sales-ledger/import', [AccountingSalesLedgerController::class, 'import'])->name('sales-ledger.import');
        Route::post('/sales-ledger/sync', [AccountingSalesLedgerController::class, 'sync'])->name('sales-ledger.sync');
        Route::post('/sales-ledger/repair-items', [AccountingSalesLedgerController::class, 'repairItems'])->name('sales-ledger.repair-items');
        Route::delete('/sales-ledger/batches/{batch}', [AccountingSalesLedgerController::class, 'destroyBatch'])->name('sales-ledger.batches.destroy');
        Route::get('/sales-ledger', [AccountingSalesLedgerController::class, 'index'])->name('sales-ledger.index');
        Route::get('/sales-ledger/{entry}/edit', [AccountingSalesLedgerController::class, 'edit'])->name('sales-ledger.edit');
        Route::put('/sales-ledger/{entry}', [AccountingSalesLedgerController::class, 'update'])->name('sales-ledger.update');
        Route::delete('/sales-ledger/{entry}', [AccountingSalesLedgerController::class, 'destroy'])->name('sales-ledger.destroy');
        ...
        Route::post('/reconciliation/bulk-cancel', [AccountingDashboardController::class, 'bulkCancelReconciliation'])->name('reconciliation.bulk-cancel');
        ...
        Route::get('/accounts/balance-history', [AccountController::class, 'balanceHistory'])->name('accounts.balance-history');
        Route::get('/accounts/adjustments', [AccountController::class, 'adjustmentHistory'])->name('accounts.adjustments');
    ...
    Route::prefix('warehouse')->name('warehouse.')->middleware('role:warehouse,admin')->group(function () {
        ...
        Route::get('/procurement-receipts', [WarehouseProcurementController::class, 'index'])->name('procurement-receipts.index');
        Route::post('/procurement-receipts/{purchase}/receive', [WarehouseProcurementController::class, 'receive'])->name('procurement-receipts.receive');
        Route::get('/order-transfers', [OrderTransferController::class, 'index'])->name('order-transfers');
        Route::post('/order-transfers', [OrderTransferController::class, 'store'])->name('order-transfers.store');
        Route::delete('/order-transfers/{id}', [OrderTransferController::class, 'destroy'])->name('order-transfers.destroy');
        Route::post('/order-transfers/{transfer}/orders/{order}/detach', [OrderTransferController::class, 'detachWaitingTransfer'])->name('order-transfers.orders.detach');
        Route::get('/dispatch-slips', [WarehouseDispatchSlipController::class, 'index'])->name('dispatch-slips.index');
        Route::get('/transfers', [WarehouseDispatchSlipController::class, 'index'])->name('transfers.index');
        Route::post('/dispatch-slips', [WarehouseDispatchSlipController::class, 'store'])->name('dispatch-slips.store');
        Route::post('/dispatch-slips/print-selected', [WarehouseDispatchSlipController::class, 'printSelected'])->name('dispatch-slips.print-selected');
        Route::get('/dispatch-slips/{dispatchSlip}/edit', [WarehouseDispatchSlipController::class, 'edit'])->name('dispatch-slips.edit');
        Route::put('/dispatch-slips/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'update'])->name('dispatch-slips.update');
        Route::get('/dispatch-slips/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'show'])->name('dispatch-slips.show');
        Route::post('/dispatch-slips/{dispatchSlip}/finalize', [WarehouseDispatchSlipController::class, 'finalize'])->name('dispatch-slips.finalize');
        Route::delete('/dispatch-slips/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'destroy'])->name('dispatch-slips.destroy');
        Route::get('/dispatch-slips/{dispatchSlip}/print-export', [WarehouseDispatchSlipController::class, 'printExport'])->name('dispatch-slips.print-export');
        Route::get('/dispatch-slips/{dispatchSlip}/print-import', [WarehouseDispatchSlipController::class, 'printImport'])->name('dispatch-slips.print-import');
        ...
        Route::get('/order-adjustments', [OrderAdjustmentController::class, 'warehouseIndex'])->name('order-adjustments.index');
        ...
        Route::get('/orders/{order}/shipping-label', [OrderShippingLabelController::class, 'show'])->name('orders.shipping-label');
        Route::post('/orders/{order}/shipping-label', [OrderShippingLabelController::class, 'update'])->name('orders.shipping-label.update');
        ...
        Route::get('/google-sheet-inventory', [WarehouseGoogleSheetInventoryController::class, 'index'])->name('google-sheet-inventory.index');
        Route::post('/google-sheet-inventory/configuration', [WarehouseGoogleSheetInventoryController::class, 'updateConfiguration'])->name('google-sheet-inventory.configuration');
        Route::post('/google-sheet-inventory', [WarehouseGoogleSheetInventoryController::class, 'store'])->name('google-sheet-inventory.store');
        Route::delete('/google-sheet-inventory/history/{sync}', [WarehouseGoogleSheetInventoryController::class, 'destroySync'])->name('google-sheet-inventory.history.destroy');
        Route::delete('/google-sheet-inventory/clear-day', [WarehouseGoogleSheetInventoryController::class, 'clearDay'])->name('google-sheet-inventory.clear-day');
        Route::get('/google-sheet-inventory-export', [WarehouseGoogleSheetInventoryController::class, 'exportIndex'])->name('google-sheet-inventory.export.index');
        Route::post('/google-sheet-inventory-export/configuration', [WarehouseGoogleSheetInventoryController::class, 'updateExportConfiguration'])->name('google-sheet-inventory.export.configuration');
        Route::post('/google-sheet-inventory-export/write-daily', [WarehouseGoogleSheetInventoryController::class, 'writeDaily'])->name('google-sheet-inventory.export.write-daily');
        Route::post('/stock-in-templates', [InventoryDocumentTemplateController::class, 'store'])->name('stock-in-templates.store');
        Route::delete('/stock-in-templates/{template}', [InventoryDocumentTemplateController::class, 'destroy'])->name('stock-in-templates.destroy');
        ...
        Route::get('/supplier-prices', [WarehouseSupplierPriceController::class, 'index'])->name('supplier-prices.index');
        Route::post('/supplier-prices', [WarehouseSupplierPriceController::class, 'store'])->name('supplier-prices.store');
        Route::post('/supplier-prices/{supplier}/{product}/apply-today-sale-price', [WarehouseSupplierPriceController::class, 'applyTodaySalePrice'])->name('supplier-prices.apply-today-sale-price');
        Route::post('/suppliers/{supplier}/products', [WarehouseSupplierPriceController::class, 'attachProduct'])->name('suppliers.products.attach');
        Route::delete('/suppliers/{supplier}/products/{product}', [WarehouseSupplierPriceController::class, 'detachProduct'])->name('suppliers.products.detach');
        ...
        Route::get('/requests', [DepartmentFinanceRequestController::class, 'warehouseIndex'])->name('finance-requests.index');
        Route::post('/requests', [DepartmentFinanceRequestController::class, 'warehouseStore'])->name('finance-requests.store');
        Route::get('/requests/{transaction}/print', [DepartmentFinanceRequestController::class, 'warehousePrint'])->name('finance-requests.print');
    ...
    Route::prefix('procurement')->name('procurement.')->middleware('role:procurement_manager,admin')->group(function () {
        ...
        Route::get('/purchases', [ProcurementController::class, 'purchases'])->name('purchases.index');
        Route::post('/purchases', [ProcurementController::class, 'storePurchase'])->name('purchases.store');
        Route::post('/purchases/import-pasted', [ProcurementController::class, 'importPastedPurchases'])->name('purchases.import-pasted');
        Route::post('/purchases/{purchase}/send-warehouse', [ProcurementController::class, 'sendToWarehouse'])->name('purchases.send-warehouse');
        Route::post('/purchases/{purchase}/request-payment', [ProcurementController::class, 'requestPayment'])->name('purchases.request-payment');
        Route::post('/purchase-templates', [ProcurementPurchaseTemplateController::class, 'store'])->name('purchase-templates.store');
        Route::delete('/purchase-templates/{template}', [ProcurementPurchaseTemplateController::class, 'destroy'])->name('purchase-templates.destroy');
        Route::get('/farms', [ProcurementController::class, 'farms'])->name('farms.index');
        Route::get('/farm-debts', [ProcurementController::class, 'farmDebts'])->name('farm-debts.index');
        Route::post('/farms', [ProcurementController::class, 'storeFarm'])->name('farms.store');
        Route::put('/farms/{farm}', [ProcurementController::class, 'updateFarm'])->name('farms.update');
        Route::delete('/farms/{farm}', [ProcurementController::class, 'destroyFarm'])->name('farms.destroy');
        Route::post('/farms/{farm}/reviews', [ProcurementController::class, 'reviewFarm'])->name('farms.reviews.store');
        Route::get('/conversions', [ProcurementController::class, 'conversions'])->name('conversions.index');
        Route::post('/conversions', [ProcurementController::class, 'storeConversions'])->name('conversions.store');
        Route::get('/warehouse-shipments', [ProcurementController::class, 'purchases'])->name('warehouse-shipments.index');
    ...
    Route::prefix('director')->name('director.')->middleware('role:director,Director,admin')->group(function () {
        ...
        Route::get('/profile', [CeoPageController::class, 'show'])->name('profile');
        Route::post('/profile', [CeoPageController::class, 'update'])->name('profile.update');
        Route::get('/change-password', [CeoChangePasswordController::class, 'show'])->name('password.change');
        Route::post('/change-password', [CeoChangePasswordController::class, 'update'])->name('password.update');
    });
    ...
    Route::prefix('ceo')->name('ceo.')->middleware('role:ceo,admin')->group(function () {
        ...
        Route::get('/warehouse/dispatch-slips', [WarehouseDispatchSlipController::class, 'ceoIndex'])->name('warehouse-dispatch-slips.index');
        Route::get('/warehouse/dispatch-slips/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'ceoShow'])->name('warehouse-dispatch-slips.show');
        Route::get('/warehouse/dispatch-slips/{dispatchSlip}/print-export', [WarehouseDispatchSlipController::class, 'ceoPrintExport'])->name('warehouse-dispatch-slips.print-export');
        Route::get('/shipper', [CeoDashboardController::class, 'shipper'])->name('shipper');
        Route::get('/shipper-costs', [CeoDashboardController::class, 'shipperCosts'])->name('shipper-costs');
        Route::get('/customers', [CeoDashboardController::class, 'customers'])->name('customers');
        Route::get('/customers-list', [CeoDashboardController::class, 'customersList'])->name('customers-list');
    ...
    Route::get('products/export', [ProductSpreadsheetController::class, 'export'])->name('products.export')->middleware('role:admin');
    Route::get('products/import-template', [ProductSpreadsheetController::class, 'template'])->name('products.import-template')->middleware('role:admin');
    Route::post('products/import', [ProductSpreadsheetController::class, 'import'])->name('products.import')->middleware('role:admin');
    ...
    Route::patch('admin/order-fee-types/{orderFeeType}/toggle', [OrderFeeTypeController::class, 'toggle'])->name('admin.order-fee-types.toggle')->middleware('role:admin');
    Route::resource('admin/order-fee-types', OrderFeeTypeController::class)
    ...
    Route::get('admin/text-order-import', [TextOrderImportController::class, 'index'])->name('admin.text-order-import.index')->middleware('role:admin');
    Route::get('admin/text-order-import/export-daily', [TextOrderImportController::class, 'exportDailyOrders'])->name('admin.text-order-import.export-daily')->middleware('role:admin');
    Route::get('admin/order-activity', [OrderActivityController::class, 'index'])->name('admin.order-activity.index')->middleware('role:admin');
    Route::get('admin/order-activity/export', [OrderActivityController::class, 'export'])->name('admin.order-activity.export')->middleware('role:admin');
    Route::post('admin/order-activity/import', [OrderActivityController::class, 'import'])->name('admin.order-activity.import')->middleware('role:admin');
    Route::get('admin/order-activity/export-full', [OrderActivityController::class, 'exportFull'])->name('admin.order-activity.export-full')->middleware('role:admin');
    Route::post('admin/order-activity/import-full', [OrderActivityController::class, 'importFull'])->name('admin.order-activity.import-full')->middleware('role:admin');
    Route::post('admin/text-order-import/parse', [TextOrderImportController::class, 'parse'])->name('admin.text-order-import.parse')->middleware('role:admin');
    Route::post('admin/text-order-import/parse-for-sale', [TextOrderImportController::class, 'parseForSale'])->name('admin.text-order-import.parse-for-sale')->middleware('role:admin');
    Route::post('admin/text-order-import/bulk-confirm', [TextOrderImportController::class, 'bulkConfirm'])->name('admin.text-order-import.bulk-confirm')->middleware('role:admin');
    Route::post('admin/text-order-import/bulk-delete', [TextOrderImportController::class, 'bulkDestroy'])->name('admin.text-order-import.bulk-delete')->middleware('role:admin');
    Route::post('admin/text-order-import/{draft}/copy', [TextOrderImportController::class, 'copy'])->name('admin.text-order-import.copy')->middleware('role:admin');
    Route::post('admin/text-order-import/{draft}/copy-confirm', [TextOrderImportController::class, 'copyConfirm'])->name('admin.text-order-import.copy-confirm')->middleware('role:admin');
    Route::post('admin/text-order-import/{draft}/confirm', [TextOrderImportController::class, 'confirm'])->name('admin.text-order-import.confirm')->middleware('role:admin');
    Route::delete('admin/text-order-import/{draft}', [TextOrderImportController::class, 'destroy'])->name('admin.text-order-import.destroy')->middleware('role:admin');
    Route::get('admin/daily-rebuild', [AdminDailyRebuildController::class, 'index'])->name('admin.daily-rebuild.index')->middleware('role:admin');
    Route::post('admin/daily-rebuild', [AdminDailyRebuildController::class, 'execute'])->name('admin.daily-rebuild.execute')->middleware('role:admin');
    Route::get('admin/google-sheet-inventory-reset', [WarehouseGoogleSheetInventoryController::class, 'resetIndex'])->name('admin.google-sheet-inventory-reset.index')->middleware('role:admin');
    Route::delete('admin/google-sheet-inventory-reset', [WarehouseGoogleSheetInventoryController::class, 'resetRange'])->name('admin.google-sheet-inventory-reset.destroy')->middleware('role:admin');
    Route::prefix('admin/warehouse-dispatch-slips')->name('admin.warehouse-dispatch-slips.')->middleware('role:admin')->group(function () {
        Route::get('/', [WarehouseDispatchSlipController::class, 'index'])->name('index');
        Route::post('/', [WarehouseDispatchSlipController::class, 'store'])->name('store');
        Route::get('/{dispatchSlip}/edit', [WarehouseDispatchSlipController::class, 'edit'])->name('edit');
        Route::put('/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'update'])->name('update');
        Route::get('/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'show'])->name('show');
        Route::post('/{dispatchSlip}/orders/{order}/remove', [WarehouseDispatchSlipController::class, 'removeOrder'])->name('orders.remove');
        Route::post('/{dispatchSlip}/mismatched-orders/remove', [WarehouseDispatchSlipController::class, 'removeMismatchedOrders'])->name('mismatched-orders.remove');
        Route::post('/{dispatchSlip}/finalize', [WarehouseDispatchSlipController::class, 'finalize'])->name('finalize');
        Route::post('/{dispatchSlip}/unfinalize', [WarehouseDispatchSlipController::class, 'unfinalize'])->name('unfinalize');
        Route::delete('/{dispatchSlip}', [WarehouseDispatchSlipController::class, 'destroy'])->name('destroy');
        Route::get('/{dispatchSlip}/print-export', [WarehouseDispatchSlipController::class, 'printExport'])->name('print-export');
        Route::get('/{dispatchSlip}/print-import', [WarehouseDispatchSlipController::class, 'printImport'])->name('print-import');
    });
    ...
    Route::post('/orders/{order}/resume-overdue-delivery', [OrderController::class, 'resumeOverdueDelivery'])
        ...
        ->middleware('role:admin');
    Route::delete('/orders/{order}/admin-delete', [AdminOrderDeletionController::class, 'destroy'])
    ...
    Route::post('task-assignments/{taskAssignment}/approve', [TaskAssignmentController::class, 'approve'])->name('task-assignments.approve');
    Route::post('task-assignments/{taskAssignment}/reject', [TaskAssignmentController::class, 'reject'])->name('task-assignments.reject');
    Route::post('task-assignments/{taskAssignment}/cancel', [TaskAssignmentController::class, 'cancel'])->name('task-assignments.cancel');
    Route::post('task-assignments/{taskAssignment}/assignee-update', [TaskAssignmentController::class, 'assigneeUpdate'])->name('task-assignments.assignee-update');
    Route::post('task-assignments/{taskAssignment}/complete-content', [TaskAssignmentController::class, 'completeWithContent'])->name('task-assignments.complete-with-content');
    Route::get('task-assignments/{taskAssignment}/complete', [TaskAssignmentController::class, 'completeForm'])->name('task-assignments.complete-form');
    Route::post('task-assignments/{taskAssignment}/verify-completion', [TaskAssignmentController::class, 'verifyCompletion'])->name('task-assignments.verify-completion');
    Route::post('task-assignments/{taskAssignment}/reject-completion', [TaskAssignmentController::class, 'rejectCompletion'])->name('task-assignments.reject-completion');
    Route::get('task-assignments/assigned/to-me', [TaskAssignmentController::class, 'assignedToMe'])->name('task-assignments.assigned-to-me');
    Route::get('task-assignments/assigned/by-me', [TaskAssignmentController::class, 'assignedByMe'])->name('task-assignments.assigned-by-me');
    Route::get('task-assignments/in-progress', [TaskAssignmentController::class, 'inProgress'])->name('task-assignments.in-progress');
    Route::get('task-assignments/awaiting-verification', [TaskAssignmentController::class, 'awaitingVerification'])->name('task-assignments.awaiting-verification');
    Route::get('task-assignments/verify-list', [TaskAssignmentController::class, 'verifyList'])->name('task-assignments.verify');
    Route::get('task-assignments/tracking', [TaskAssignmentController::class, 'verifyList'])->name('task-assignments.tracking');
    Route::get('task-assignments/history', [TaskAssignmentController::class, 'history'])->name('task-assignments.history');
    Route::resource('task-assignments', TaskAssignmentController::class);

    // My Tasks - cho Sale/Leader/Manager users
    Route::get('/my-tasks', [TaskAssignmentController::class, 'assignedToMe'])->name('my-tasks');

    // User-facing task aliases
    Route::get('/tasks', [TaskAssignmentController::class, 'index'])->name('tasks.index');
    Route::get('/tasks/create', [TaskAssignmentController::class, 'create'])->name('tasks.create');
    Route::post('/tasks', [TaskAssignmentController::class, 'store'])->name('tasks.store');
    Route::get('/tasks/my-tasks', [TaskAssignmentController::class, 'assignedToMe'])->name('tasks.my-tasks');
    Route::get('/tasks/assigned', [TaskAssignmentController::class, 'assignedByMe'])->name('tasks.assigned');
    Route::get('/tasks/in-progress', [TaskAssignmentController::class, 'inProgress'])->name('tasks.in-progress');
    Route::get('/tasks/awaiting-verification', [TaskAssignmentController::class, 'awaitingVerification'])->name('tasks.awaiting-verification');
    Route::get('/tasks/verify', [TaskAssignmentController::class, 'verifyList'])->name('tasks.verify');
    Route::get('/tasks/rejected', [TaskAssignmentController::class, 'history'])->name('tasks.rejected');
    Route::get('/tasks/{taskAssignment}', [TaskAssignmentController::class, 'show'])->name('tasks.show');
    Route::post('/tasks/{taskAssignment}/complete', [TaskAssignmentController::class, 'completeWithContent'])->name('tasks.complete');
    Route::get('/tasks/{taskAssignment}/complete', [TaskAssignmentController::class, 'completeForm'])->name('tasks.complete-form');
    Route::post('/tasks/{taskAssignment}/verify', [TaskAssignmentController::class, 'verifyCompletion'])->name('tasks.verify-completion');
    Route::post('/tasks/{taskAssignment}/reject', [TaskAssignmentController::class, 'rejectCompletion'])->name('tasks.reject-completion');

    // Phân quyền giao việc (Admin only)
    Route::post('task-delegate-configs/{taskDelegateConfig}/toggle', [TaskDelegateConfigController::class, 'toggle'])->name('task-delegate-configs.toggle');
    Route::post('task-delegate-configs/destroy-assigner', [TaskDelegateConfigController::class, 'destroyAssigner'])->name('task-delegate-configs.destroy-assigner');
    Route::resource('task-delegate-configs', TaskDelegateConfigController::class)->only(['index', 'create', 'store', 'destroy']);
    ...
    Route::resource('companies', CompanyController::class)->middleware('permission');
    Route::get('companies/export', [CompanyController::class, 'export'])->name('companies.export')->middleware('permission');
    Route::get('companies/import', [CompanyController::class, 'importForm'])->name('companies.import.form')->middleware('permission');
    Route::post('companies/import', [CompanyController::class, 'import'])->name('companies.import')->middleware('permission');
    ...
    Route::group(['prefix' => 'admin', 'as' => 'admin.'], function () {
        ...
        Route::resource('brands', BrandController::class)->middleware('permission');
        Route::resource('suppliers', SupplierController::class)->only(['index', 'store', 'update', 'destroy'])->middleware('permission');

        // Truck management
        Route::get('truck-brands/{truckBrand}/routes', [TruckBrandController::class, 'routes'])->name('truck-brands.routes');
        Route::resource('truck-brands', TruckBrandController::class);
        Route::resource('truck-stations', TruckStationAdminController::class);
        Route::get('truck-routes/stations/search', [TruckRouteController::class, 'searchStations'])->name('truck-routes.stations.search');
        Route::post('truck-routes/{truckRoute}/stops', [TruckRouteController::class, 'storeStop'])->name('truck-routes.stops.store');
        Route::put('truck-routes/{truckRoute}/stops/{stop}', [TruckRouteController::class, 'updateStop'])->name('truck-routes.stops.update');
        Route::delete('truck-routes/{truckRoute}/stops/{stop}', [TruckRouteController::class, 'destroyStop'])->name('truck-routes.stops.destroy');
        Route::resource('truck-routes', TruckRouteController::class);
    });
    ...
    Route::post('/site/orders/{order}/approve', [OrderApprovalController::class, 'approve'])->name('site.orders.approve');
    Route::post('/site/orders/{order}/reject', [OrderApprovalController::class, 'reject'])->name('site.orders.reject');
...
Route::middleware(['auth', 'role:sale,leader,leader_sale,sale_manager,manager,manager_sale,director,admin'])->group(function () {
    Route::get('/my-order-drafts', [TextOrderImportController::class, 'saleIndex'])->name('pages.my_order_drafts')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts', [TextOrderImportController::class, 'saleStore'])->name('pages.my_order_drafts.store')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts/customers/{customer}/pin', [TextOrderImportController::class, 'updateCustomerPin'])->name('pages.my_order_drafts.customers.pin')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::put('/my-order-drafts/{draft}', [TextOrderImportController::class, 'saleUpdate'])->name('pages.my_order_drafts.update')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::put('/my-order-drafts/{draft}/automation', [TextOrderImportController::class, 'saleUpdateAutomation'])->name('pages.my_order_drafts.automation')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts/bulk-confirm', [TextOrderImportController::class, 'saleBulkConfirm'])->name('pages.my_order_drafts.bulk_confirm')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts/{draft}/copy', [TextOrderImportController::class, 'saleCopy'])->name('pages.my_order_drafts.copy')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts/{draft}/copy-confirm', [TextOrderImportController::class, 'saleCopyConfirm'])->name('pages.my_order_drafts.copy-confirm')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::post('/my-order-drafts/{draft}/confirm', [TextOrderImportController::class, 'saleConfirm'])->name('pages.my_order_drafts.confirm')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    Route::delete('/my-order-drafts/{draft}', [TextOrderImportController::class, 'saleDestroy'])->name('pages.my_order_drafts.destroy')->middleware('role:sale,leader,leader_sale,sale_manager,manager,manager_sale,admin');
    ...
    Route::post('/my-orders/monitoring/order-review/google-sheets', [GoogleSheetsOrderReviewController::class, 'store'])
        ->name('pages.my_orders.monitoring.order_review.google_sheets');
    ...
    Route::get('/my-orders/fix-data', [OrderAdjustmentController::class, 'index'])
    ...
    Route::get('/my-orders/variants/ajax', [OrderAjaxController::class, 'variantsAjax'])->name('site.orders.variants.ajax');
    ...
    Route::delete('/my-orders/{order}/admin-delete', [AdminOrderDeletionController::class, 'destroy'])
    ...
    Route::post('/my-orders/{order}/add-to-samples', [TextOrderImportController::class, 'saleAddFromOrder'])
    ...
    Route::get('/my-orders/{order}/adjustments/create', [OrderAdjustmentController::class, 'create'])->name('site.order-adjustments.create');
    Route::post('/my-orders/{order}/adjustments', [OrderAdjustmentController::class, 'store'])->name('site.order-adjustments.store');
...
Route::middleware(['auth'])->group(function () {
    Route::get('/my-order-adjustments/{orderAdjustment}', [OrderAdjustmentController::class, 'show'])->name('site.order-adjustments.show');
    Route::post('/my-order-adjustments/{orderAdjustment}/approve', [OrderAdjustmentController::class, 'approve'])->name('site.order-adjustments.approve');
    Route::post('/my-order-adjustments/{orderAdjustment}/reject', [OrderAdjustmentController::class, 'reject'])->name('site.order-adjustments.reject');
    Route::post('/my-order-adjustments/{orderAdjustment}/warehouse-confirm', [OrderAdjustmentController::class, 'warehouseConfirm'])->name('site.order-adjustments.warehouse-confirm');
    Route::delete('/my-order-adjustments/{orderAdjustment}', [OrderAdjustmentController::class, 'destroy'])->name('site.order-adjustments.destroy');
});
