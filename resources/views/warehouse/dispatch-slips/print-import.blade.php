...
@forelse($orderRows as $row)@php($out=$row['packed_weight']) @php($received=$row['movement']?->received_total_weight)<tr><td class="center">{{ $loop->iteration }}</td><td><strong>{{ $row['code'] }}</strong><br>{{ $row['customer_name'] }}</td><td class="num">{{ $formatKg($out) }}</td><td class="num">{{ $received === null ? '—' : $formatKg($received) }}</td><td class="num {{ $received !== null && abs($out-(float)$received) >= .001 ? 'danger' : '' }}">{{ $received === null ? '—' : $formatKg($out-(float)$received) }}</td><td>{{ $row['received'] ? 'Đã tiếp nhận' : 'Chưa tiếp nhận' }}</td></tr>@empty<tr><td colspan="6" class="center">Không có đơn hoàn thiện.</td></tr>@endforelse
</tbody></table>
...
@foreach($summaryRows as $row)@php($weightDiff=$row['received_weight']===null?null:$row['weight']-$row['received_weight'])<tr><td>{{ $row['product_name'] }}</td><td>{{ $row['sku'] ?: '—' }} / {{ $row['size'] ?: '—' }}</td><td class="num">{{ number_format($row['quantity']) }}</td><td class="num">{{ $row['received_quantity']===null?'—':number_format($row['received_quantity']) }}</td><td class="num">{{ $formatKg($row['weight']) }}</td><td class="num">{{ $row['received_weight']===null?'—':$formatKg($row['received_weight']) }}</td><td class="num {{ $weightDiff !== null && abs($weightDiff)>=.001?'danger':'' }}">{{ $weightDiff===null?'—':$formatKg($weightDiff) }}</td></tr>@endforeach
</tbody></table>
