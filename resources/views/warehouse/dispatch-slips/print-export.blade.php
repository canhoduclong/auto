...
@forelse($orderRows as $row)<tr><td class="center">{{ $loop->iteration }}</td><td>{{ $row['code'] }}</td><td>{{ $row['customer_name'] }}</td><td>{{ $row['sale_name'] }}</td><td class="num">{{ number_format($row['item_quantity']) }}</td><td class="num">{{ number_format($row['packed_weight'],3,',','.') }} kg</td></tr>@empty<tr><td colspan="6" class="center">Không có đơn hoàn thiện.</td></tr>@endforelse
</tbody></table>
...
@foreach($summaryRows as $row)<tr><td class="center">{{ $loop->iteration }}</td><td>{{ $row['product_name'] }}</td><td>{{ $row['sku'] ?: '—' }}</td><td>{{ $row['size'] ?: '—' }}</td><td class="num">{{ number_format($row['quantity']) }}</td><td class="num">{{ number_format($row['weight'],3,',','.') }} kg</td></tr>@endforeach
</tbody><tfoot><tr><th colspan="4">Tổng cộng</th><th class="num">{{ number_format($summaryRows->sum('quantity')) }}</th><th class="num">{{ number_format((float)$summaryRows->sum('weight'),3,',','.') }} kg</th></tr></tfoot></table>
