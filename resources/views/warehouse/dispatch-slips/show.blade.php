...
<div class="card mb-3"><div class="card-header bg-white fw-bold">Danh sách đơn hoàn thiện</div><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead class="table-light"><tr><th>STT</th><th>Khách hàng / Mã đơn</th><th>Sale</th><th class="text-end">Số lượng hàng</th><th class="text-end">KL bàn giao</th><th class="text-end">KL nhận</th><th>Trạng thái</th></tr></thead><tbody>
@forelse($orderRows as $row)<tr><td>{{ $loop->iteration }}</td><td><div class="fs-5 fw-bold lh-sm text-dark">{{ $row['customer_name'] ?: 'Không rõ khách hàng' }}</div><div class="small fw-normal text-muted mt-1">Mã đơn: {{ $row['code'] }}</div></td><td>{{ $row['sale_name'] }}</td><td class="text-end fw-bold text-nowrap">{{ number_format($row['item_quantity']) }}</td><td class="text-end fw-semibold text-nowrap">{{ $formatKg($row['packed_weight']) }}</td><td class="text-end text-nowrap">{{ $row['movement']?->received_total_weight !== null ? $formatKg($row['movement']->received_total_weight) : '—' }}</td><td><span class="badge {{ $row['received'] ? 'bg-success' : 'bg-warning text-dark' }}">{{ $row['received'] ? 'Đã tiếp nhận' : 'Chưa tiếp nhận' }}</span></td></tr>@empty<tr><td colspan="7" class="text-center text-muted py-3">Phiếu không có đơn hoàn thiện.</td></tr>@endforelse
</tbody></table></div></div>
