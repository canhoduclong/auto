...
<div class="container-fluid py-3">
    ...
    @elseif($preview)
        ...
        @if($existingDocument)
            <div class="alert alert-warning d-flex flex-wrap justify-content-between align-items-center gap-2"><span><i class="bi bi-shield-check me-1"></i>Dữ liệu ngày này đã được nhập bằng phiếu <strong>{{ $existingDocument->document_number }}</strong>. Hệ thống đã khóa nhập trùng.</span><a href="{{ route('warehouse.stock-in.show', $existingDocument) }}" class="btn btn-sm btn-outline-dark">Xem phiếu</a></div>
        @endif
        ...
        <div class="card sheet-import-card"><div class="card-body">
            <div class="alert alert-info small"><i class="bi bi-info-circle me-1"></i>Thao tác này tạo <strong>phiếu nhập kho</strong> với ngày chứng từ {{ \Carbon\Carbon::parse($selectedDate)->format('d/m/Y') }} và cộng số lượng cột Tồn vào tồn hiện tại của <strong>{{ $warehouse->name }}</strong>. Mỗi ngày/kho chỉ được nhập một lần.</div>
            <form method="POST" action="{{ route('warehouse.google-sheet-inventory.store') }}" onsubmit="return confirm('Xác nhận tạo phiếu nhập và cộng {{ number_format($preview['total_quantity'], 0, ',', '.') }} vào {{ $warehouse->name }}?')">@csrf
                ...
                <button class="btn btn-success btn-lg" @disabled($existingDocument || $preview['import_rows']->isEmpty())><i class="bi bi-box-arrow-in-down me-1"></i>Thực hiện nhập kho</button>
            </form>
        </div></div>
    @endif
</div>
