    ...
    <div class="modal fade js-cutting-modal"
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable cutting-order-dialog">
            <div class="modal-content">
                <form method="POST" action="{{ route('warehouse.cutting.confirm', ['variant' => $targetVariantId]) }}" data-execute-url="{{ route('warehouse.cutting.execute', ['variant' => $targetVariantId]) }}" class="js-cutting-execute-form">
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <fieldset disabled class="cutting-actual-panel mt-3 js-cutting-actual-panel d-none">
                            ...
                            <div class="row g-2 mb-2">
                                <div class="col-md-4">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <input type="number" name="actual_finished_weight" required step="0.001" min="0.001" class="form-control js-cutting-actual-finished-input" value="{{ (float) data_get($plan, 'preview.finished_weight', 0) }}">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary js-cutting-start d-none">Thực hiện</button>
                        <button type="button" class="btn btn-outline-secondary js-cutting-edit d-none">Quay lại</button>
                        <button type="submit" class="btn btn-success js-cutting-complete d-none">Hoàn thành</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function renderCuttingPreview(modal) {
                    ...
                    modal.querySelectorAll('.js-cutting-start').forEach((confirmButton) => {
                        const canConfirm = selectedRows.length > 0 && finishedWeight > 0;
                        confirmButton.classList.toggle('d-none', !canConfirm || modal.classList.contains('is-cutting-in-progress'));
                ...
                function updateCuttingLossPreview(modal) {
                    ...
                    const finishedInput = modal.querySelector('.js-cutting-actual-finished-input');
                    finishedInput?.setCustomValidity(
                        Math.round((finishedWeight + componentWeight) * 1000) > Math.round(inputWeight * 1000)
                            ? 'Tổng khối lượng thực tế không được vượt quá khối lượng nguyên liệu.'
                            : ''
                    );
                ...
                document.querySelectorAll('.js-cutting-modal').forEach((modal) => {
                    ...
                    const materialOptions = plan.material_options || plan.materials || [];
                    const materialsById = new Map(materialOptions.map((material) => [String(material.variant_id), material]));
                    const form = modal.querySelector('.js-cutting-execute-form');
                    const confirmUrl = form.action;

                    function setCuttingInProgress(inProgress) {
                        modal.classList.toggle('is-cutting-in-progress', inProgress);
                        form.action = inProgress ? form.dataset.executeUrl : confirmUrl;
                        const panel = modal.querySelector('.js-cutting-actual-panel');
                        panel.disabled = !inProgress;
                        panel.classList.toggle('d-none', !inProgress);
                        modal.querySelectorAll('.js-cutting-complete, .js-cutting-edit').forEach((button) => {
                            button.classList.toggle('d-none', !inProgress);
                        });
                        modal.querySelector('.js-cutting-build-preview').classList.toggle('d-none', inProgress);
                        modal.querySelectorAll('.js-cutting-material-check, .js-cutting-material-select, .js-cutting-material-add, .js-cutting-material-remove').forEach((input) => {
                            input.disabled = inProgress;
                        });
                        modal.querySelectorAll('.js-cutting-material-qty').forEach((input) => {
                            input.readOnly = inProgress;
                        });
                        refreshCuttingPreview();
                        if (inProgress) {
                            panel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                            panel.querySelector('.js-cutting-actual-finished-input').focus({ preventScroll: true });
                        }
                    }

                    modal.querySelector('.js-cutting-start').addEventListener('click', function () {
                        refreshCuttingPreview();
                        if (form.reportValidity() && !this.disabled) {
                            setCuttingInProgress(true);
                        }
                    });
                    modal.querySelector('.js-cutting-edit').addEventListener('click', function () {
                        setCuttingInProgress(false);
                    });
                    form.addEventListener('submit', function (event) {
                        if (!modal.classList.contains('is-cutting-in-progress') || form.dataset.submitting === '1' || cuttingNumber(modal.dataset.plannedFinishedWeight) <= 0) {
