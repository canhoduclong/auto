...
<div class="card-body">@foreach($transfer->items as $item)<div class="d-flex justify-content-between border-bottom py-2 gap-2"><span>{{ $item->variant?->product?->name }} - {{ $item->variant?->name }}</span><strong class="text-nowrap">{{ number_format($item->quantity) }} · {{ number_format((float) $item->weight_kg, 3, ',', '.') }} kg</strong></div>@endforeach<div class="text-end fw-bold text-primary pt-2">Tổng KL xuất: {{ number_format((float) $transfer->items->sum('weight_kg'), 3, ',', '.') }} kg</div></div>
