            ...
            <div class="fee-admin-card-body">
                <form method="POST" action="{{ route('admin.order-fee-types.store') }}">
                    ...
                    <div class="row g-2 mb-3">
                        ...
                        <div class="col-6"><label class="form-label fw-semibold">Tác động</label><select name="direction" class="form-select"><option value="charge" @selected(old('direction') === 'charge')>Cộng thêm</option><option value="discount" @selected(old('direction') === 'discount')>Giảm trừ</option></select><div class="form-text">Cộng thêm làm tăng tổng đơn; giảm trừ làm giảm tổng đơn.</div></div>
                    </div>
                    <div class="row g-2 mb-3">
    ...
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <form method="POST" action="{{ route('admin.order-fee-types.update', $type) }}">
            ...
            <div class="modal-body"><div class="row g-3">
                ...
                <div class="col-md-4"><label class="form-label fw-semibold">Cách tính</label>@if($type->is_system)<input type="hidden" name="calculation_type" value="{{ $type->calculation_type }}">@endif<select name="calculation_type" class="form-select" @disabled($type->is_system)><option value="fixed" @selected($type->calculation_type === 'fixed')>Số tiền</option><option value="percent" @selected($type->calculation_type === 'percent')>Phần trăm</option></select></div>
                <div class="col-md-4"><label class="form-label fw-semibold">Tác động</label>@if($type->is_system)<input type="hidden" name="direction" value="{{ $type->direction }}">@endif<select name="direction" class="form-select" @disabled($type->is_system)><option value="charge" @selected($type->direction === 'charge')>Cộng thêm</option><option value="discount" @selected($type->direction === 'discount')>Giảm trừ</option></select><div class="form-text">{{ $type->is_system ? 'Phí hệ thống có tác động cố định.' : 'Quyết định dấu của khoản phí trên tổng đơn.' }}</div></div>
