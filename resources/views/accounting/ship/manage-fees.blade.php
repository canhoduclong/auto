...
<div class="mf-filter-group">
    <form method="GET" action="{{ route('accounting.shipping-costs') }}" class="d-flex gap-2 align-items-center flex-grow-1">
        <input type="date" name="date" value="{{ $selectedDate }}" class="form-control form-control-sm" style="max-width: 150px">
        <button type="submit" class="btn btn-sm btn-primary">
            <i class="bi bi-search me-1"></i>Lọc
        </button>
        <a href="{{ route('accounting.shipping-costs') }}" class="btn btn-sm btn-outline-secondary">
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        <div class="row g-3 align-items-end">
            ...
            <div class="col-md-6">
                <form id="shippingFeeRequestForm" method="POST" action="{{ route('accounting.shipping-costs.shipping-fee-requests.store') }}" onsubmit="return confirm('Tạo phiếu yêu cầu từ các đơn đã chọn? Sau khi tạo, phí ship của các đơn này sẽ bị khóa.');">
                    @csrf
...
<div class="mf-bulk-section">
    ...
    <form id="bulkFeeForm" action="{{ route('accounting.shipping-costs.bulk-update-fees') }}" method="POST">
        @csrf
        <div class="row g-2">
            <div class="col-md-2">
    ...
    @foreach($orders as $order)
        ...
        <div class="mf-order-row {{ $alreadyRequested ? 'is-requested' : '' }}">
            ...
            <div>
                ...
                <form action="{{ route('accounting.shipping-costs.update-fee', $order->id) }}" method="POST" class="d-inline-block w-100">
                    @csrf
                        ...
                        <tbody>
                            @foreach($orderReturns as $return)
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('accounting.shipping-costs.update-return-fee', $return) }}" method="POST" class="row g-2 align-items-center">
                                            @csrf
                                            <div class="col-5">
                                                <input
                                                    type="number"
