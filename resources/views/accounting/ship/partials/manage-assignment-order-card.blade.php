                ...
                <div class="ma-assignment-picker-actions d-flex flex-column gap-1">
                    @if($customer?->defaultShipper)
                        <button type="button"
                            ...
                            data-action="{{ route('accounting.shippers.customers.default-shipper.update', $customer) }}"
                    ...
                    <button type="button"
                        ...
                        data-action="{{ route('accounting.shippers.assign-order.selected', $order) }}"
        ...
        @if(($showAssignmentButtons ?? false) === false)
            <div class="ma-shipper-actions d-flex flex-wrap gap-2 align-items-center">
                ...
                @if($order->shipper_id)
                    ...
                    <form action="{{ route('accounting.shippers.unassign-order', [$order->id]) }}" method="POST">
                        @csrf
                        @method('DELETE')
                    ...
                    <form action="{{ route('accounting.shippers.move-order-up', [$order->id]) }}" method="POST">
                        @csrf
                    ...
                    <form action="{{ route('accounting.shippers.move-order-down', [$order->id]) }}" method="POST">
                        @csrf
                        <input type="hidden" name="date" value="{{ $selectedDate ?? now()->toDateString() }}">
