            ...
            <tr class="collapse" id="supplierDebt{{ $debt['key'] }}"><td colspan="8" class="bg-light p-3">
                <div class="table-responsive"><table class="table table-sm align-middle mb-0 supplier-detail-table">
                    ...
                    @foreach($debt['purchases'] as $purchase)
                        ...
                        @endphp
                        <tr>
                            <td>{{ $purchase->purchased_at->format('d/m/Y') }}<div class="fw-semibold">{{ $purchase->code }}</div><div class="small text-muted">{{ $purchase->purchase_type === 'live_duck' ? 'Vịt lông' : 'Vịt sơ chế' }}</div></td>
                            <td>{{ number_format($purchase->quantity) }} con<div class="small text-muted">{{ number_format($purchase->total_weight, 1) }} kg · {{ number_format($purchase->unit_price) }}đ/kg</div></td>
