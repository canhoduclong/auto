...
@forelse($purchases as $p)
    <tr><td>{{ $p->purchased_at->format('d/m/Y H:i') }}<div class="fw-bold">{{ $p->code }}</div></td><td>{{ $p->entry_mode === 'product_lines' ? 'Theo sản phẩm' : ($p->purchase_type === 'live_duck' ? 'Vịt lông' : 'Vịt thịt') }}<div class="small text-muted">{{ $p->farm?->name ?? $p->supplier?->name }}</div><div class="small">{{ $p->entry_mode === 'product_lines' ? $p->productItems->count().' mặt hàng' : (($p->duck_type ?: '—').' · Trại '.($p->farm_type ?: '—')) }}</div></td><td>{{ number_format($p->quantity) }} đơn vị<div class="small">{{ number_format($p->total_weight, 1) }}kg @if($p->entry_mode !== 'product_lines')· TB {{ number_format($p->average_weight, 2) }}@endif</div></td><td>{{ $p->notes ?: ($p->duck_condition ?: '—') }}</td><td class="text-end"><div class="fw-bold">{{ number_format($p->total_amount) }}đ</div><div class="small text-muted">Thu mua: {{ number_format($p->procurement_fee) }}đ</div><div class="small text-muted">Vận chuyển: {{ number_format($p->transportation_fee) }}đ</div></td><td><span class="badge {{ $p->payment_status === 'paid' ? 'bg-success' : 'bg-warning text-dark' }}">{{ $p->payment_status }}</span><div class="small text-success">Đã trả: {{ number_format($p->paid_amount) }}đ</div><div class="small text-danger">Còn: {{ number_format($p->remaining_amount) }}đ</div>@if($p->payment_due_date)<div class="small">Hạn: {{ $p->payment_due_date->format('d/m/Y') }}</div>@endif @if($p->payment_transaction_id)<div class="small">Phiếu #{{ $p->payment_transaction_id }}</div>@endif</td><td><span class="badge bg-secondary">{{ $p->status }}</span><div class="small">{{ $p->warehouse?->name }}</div>@if($p->received_at)<div class="small text-success">{{ $p->received_at->format('d/m H:i') }} · {{ $p->warehouse_rating }}★</div><div class="small text-muted">{{ $p->warehouse_condition }}</div>@endif</td><td>@if(!$p->payment_transaction_id && (float) $p->remaining_amount > 0)<button type="button" class="btn btn-sm btn-outline-danger js-preview-payment" data-action="{{ route('procurement.purchases.request-payment', $p) }}" data-code="{{ $p->code }}" data-source="{{ $p->farm?->name ?? $p->supplier?->name ?? 'Nhà cung cấp' }}" data-date="{{ $p->purchased_at->format('d/m/Y H:i') }}" data-description="{{ $p->entry_mode === 'product_lines' ? 'Thu mua '.$p->productItems->count().' sản phẩm' : ($p->purchase_type === 'live_duck' ? 'Vịt lông' : 'Vịt thịt').' · '.number_format($p->quantity).' con · '.number_format($p->total_weight, 1).'kg' }}" data-subtotal="{{ (float) $p->subtotal }}" data-broker="{{ (float) $p->broker_fee }}" data-processing="{{ (float) $p->processing_fee }}" data-procurement="{{ (float) $p->procurement_fee }}" data-transportation="{{ (float) $p->transportation_fee }}" data-other="{{ (float) $p->other_fee }}" data-total="{{ (float) $p->total_amount }}" data-paid="{{ (float) $p->paid_amount }}" data-remaining="{{ (float) $p->remaining_amount }}" data-due="{{ $p->payment_due_date?->format('d/m/Y') ?? 'Chưa đặt hạn' }}" data-notes="{{ $p->notes ?: 'Không có ghi chú' }}">Yêu cầu thanh toán</button>@else — @endif</td></tr>
    <tr class="table-light"><td colspan="8"><strong>{{ $p->entry_mode === 'product_lines' ? 'Sản phẩm:' : 'Dự kiến/Phân size:' }}</strong> @if($p->entry_mode === 'product_lines')@foreach($p->productItems as $i)<span class="badge bg-light text-dark border me-1">{{ $i->productVariant?->product?->name }} - {{ $i->productVariant?->name }}: {{ number_format((float)$i->quantity, 3) }} {{ $i->productVariant?->product?->unit_label }} · {{ number_format($i->line_total) }}đ</span>@endforeach @else @foreach($p->items->where('stage', 'expected') as $i)<span class="badge bg-light text-dark border me-1">{{ $i->item_type === 'processed_duck' ? 'Size '.$i->size : ($i->item_type === 'feathers' ? 'Lông' : 'Lòng') }}: {{ number_format($i->quantity) }}</span>@endforeach @endif @if($p->warehouse_comment)<div class="mt-1"><strong>Kho đánh giá:</strong> {{ $p->warehouse_condition }} — {{ $p->warehouse_comment }}</div>@endif</td></tr>
@empty
