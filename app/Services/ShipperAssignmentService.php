...
class ShipperAssignmentService
{
    ...
    public function publishDailySchedule(
        ...
        $latestHistory = OrderHistory::query()
            ...
            ->whereDate('orders.delivery_date', $dateString)
        ...
        $routeChanged = $latestHistory !== null
            && $latestHistory->schedule_snapshot_hash !== $snapshotHash;
        $note = $routeChanged
            ? 'Lộ trình giao hàng tự động được cập nhật. Vui lòng kiểm tra và xác nhận.'
            : 'Lộ trình giao hàng tự động được tạo. Vui lòng kiểm tra và xác nhận.';
