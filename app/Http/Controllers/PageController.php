...
use App\Imports\CustomerImportWithErrorReport;
...
use App\Support\OrderPriceBounds;
...
class PageController extends Controller
{
    ...
    public function productsByCategory(Request $request, ?Category $category = null)
    {
        ...
        if ($category) {
            $query->whereHas('product', fn ($q) => $q->where('category_id', $category->id)->where('status', true)
            );
        }
    ...
    public function myOrdersMonitoring(Request $request)
    {
        ...
        $pendingAdjustmentRequests = app(ApprovalService::class)
            ...
            ->filter(fn ($adjustment): bool => $adjustment->order
        ...
        $adjustmentApprovalRoleLabel = $pendingAdjustmentRequests
            ->map(fn ($adjustment) => OrderAdjustment::approvalRoleLabel(
    ...
    private function monitoringTabResponse(string $tab, Request $request)
    {
        ...
        return match ($tab) {
            'drafts' => app(TextOrderImportController::class)->saleIndex($tabRequest),
    ...
    public function teamOrderDetail(Order $order)
    {
        ...
        $customerDebt = null;
        if ($customer) {
            $customerDebt = Order::where('customer_id', $customer->id)
    ...
    public function teamOrderCustomerOrders(Order $order, Request $request): JsonResponse
    {
        if (! auth()->check()) {
    ...
    public function myCustomerAjax(Request $request)
    {
        ...
        \Log::info('DEBUG_PRIORITY_46', [
            'priority' => CustomerPriority::where('customer_id', 46)
    ...
    public function myCustomerImport(Request $request)
    {
        ...
        $import = new CustomerImportWithErrorReport;
    ...
    public function myCustomerOrderStore(Request $request, Customer $customer, ApprovalService $approvalService)
    {
        ...
        return app(OrderController::class)->storeANewOrder($request, $approvalService);
    }
    ...
    public function myOrderUpdate(Request $request, Order $order)
    {
        ...
        foreach ($itemsInput as $item) {
            ...
            $minPrice = OrderPriceBounds::minimum((float) ($variant->latestPriceRule?->min_price ?? 0));
        ...
        DB::transaction(function () use ($order, $validated, $itemsInput, $variants, $isCopiedOrder, $isReturnOrder): void {
            $saleChangeService = app(WarehouseSaleChangeService::class);
            ...
            $orderUpdateData = [
                ...
                'warehouse_product_permissions' => $validated['warehouse_product_permissions'] ?? null,
                'warehouse_can_adjust' => (bool) ($validated['warehouse_can_adjust'] ?? false),
                'warehouse_allowed_sizes' => $validated['warehouse_allowed_sizes'] ?? [],
                'total_discount' => $totalDiscount,
            ...
            app(ApprovalService::class)->initOrderApproval(
                $order->fresh(),
                $isReturnOrder
                    ? ApprovalWorkflow::ACTIVITY_ORDER_RETURN
                    : ApprovalWorkflow::ACTIVITY_ORDER_CREATE
            );
        });
    ...
    public function myTruckStationsAjax(Request $request)
    {
        ...
        if ($mode === 'routes') {
            $routeQuery = TruckRoute::query()->with([
                'brand',
                'stops.station',
            ]);
    ...
    private function buildStockWarnings(\Illuminate\Support\Collection $pageOrders): array
    {
        ...
        // Variant name cache
        $variantNames = ProductVariant::whereIn('id', $variantIds)
            ->pluck('name', 'id')
            ->toArray();
