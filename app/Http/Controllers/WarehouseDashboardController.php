...
use App\Models\InventoryDocumentEdit;
...
use App\Services\WarehousePackedOrderService;
...
class WarehouseDashboardController extends Controller
{
    ...
    public function allNotifications(Request $request)
    {
        ...
        $saleConfirmOrders = Order::query()
            ->with('customer')
            ->where('warehouse_id', $warehouseId)
            ->where('warehouse_adjustment_status', Order::WAREHOUSE_ADJUSTMENT_STATUS_SALE_CONFIRMED)
        ...
        $pendingSaleConfirmOrders = Order::query()
            ->with('customer')
            ->where('warehouse_id', $warehouseId)
        ...
        $transferToWarehouse = WarehouseTransfer::query()
        ...
        $transferCheck = WarehouseTransfer::query()
    ...
    public function orders(Request $request)
    {
        ...
        $ordersQuery = Order::with([
            ...
            ->where(fn ($query) => $query->whereIn('status', self::PACKING_PAGE_STATUSES)
                ->orWhereHas('histories', fn ($history) => $history->where('action', WarehouseSaleChangeService::CHANGED)))
            ->whereNull('trash_at')
            ->where(function ($query) {
                $query->whereNull('is_return_order')
                    ->orWhere('is_return_order', false);
        ...
        app(WarehouseSaleChangeService::class)->attach($orders);
    ...
    public function pullTransferToWarehouse(Request $request, WarehouseTransfer $transfer)
    {
        ...
        DB::transaction(function () use ($transfer, $validated): void {
            ...
            if ($locked->status !== WarehouseTransfer::STATUS_IN_TRANSIT) {
                throw ValidationException::withMessages(['transfer' => 'Đơn vừa được cập nhật bởi người khác.']);
            }
            $locked->update([
    ...
    public function confirmSaleChanges(Request $request, Order $order)
    {
        ...
        DB::transaction(function () use ($order, $validated): void {
            ...
            $change = $locked->histories()->where('action', WarehouseSaleChangeService::CHANGED)
                ->whereKey($validated['through_id'])->firstOrFail();
            $alreadyConfirmed = $locked->histories()->where('action', WarehouseSaleChangeService::CONFIRMED)
                ->get()->contains(fn ($history) => (int) $history->schedule_snapshot_hash >= $change->id);
            if (! $alreadyConfirmed) {
                OrderHistory::create([
                    'order_id' => $locked->id, 'action' => WarehouseSaleChangeService::CONFIRMED,
    ...
    /** Save the main/adjacent-size mix used to fulfil a short-stock order line. */
    public function updatePackingSizeAllocation(Request $request, Order $order)
    {
        $this->authorizePackingOrderAccess($order);

        if (! $this->canProcessOrderOnCurrentRun($order)) {
            throw ValidationException::withMessages(['allocations' => 'Chỉ được bổ sung cơ cấu size cho đơn của ngày hôm nay.']);
        }
        if (! in_array($order->status, array_merge(self::READY_TO_PACK_STATUSES, [Order::STATUS_PACKING]), true)) {
            throw ValidationException::withMessages(['allocations' => 'Chỉ được bổ sung size khi đơn đang chờ hoặc đang đóng hàng.']);
        }
        ...
        if (! $item || $mainSize <= 0) {
            throw ValidationException::withMessages(['allocations' => 'Dòng hàng không có size hợp lệ để chọn size liền kề.']);
        }
        if (! ($item->variant->product?->allow_adjacent_packing_sizes ?? true)) {
            throw ValidationException::withMessages(['allocations' => 'Sản phẩm này đã tắt chức năng chọn size liền kề khi đóng hàng.']);
        }
        ...
        if ($variants->count() !== $allocationInput->count()
            ...
            throw ValidationException::withMessages(['allocations' => 'Chỉ được dùng size chính '.$mainSizeLabel.' hoặc size khác của cùng sản phẩm.']);
        }

        $warehouseId = (int) ($request->user()?->warehouse_id ?: $order->warehouse_id ?: 0);
        if ($warehouseId <= 0) {
            throw ValidationException::withMessages(['allocations' => 'Không xác định được kho đang đóng hàng.']);
        }
        ...
        if ((int) $allocationInput->sum() !== $orderedQuantity) {
            throw ValidationException::withMessages(['allocations' => "Tổng số lượng đóng phải bằng {$orderedQuantity} sản phẩm của đơn."]);
        }
        ...
        if ($orderedQuantity <= 0) {
            throw ValidationException::withMessages(['allocations' => 'Tổng số lượng đóng phải lớn hơn 0.']);
        }
        ...
        OrderHistory::create([
            ...
            'status_before' => $order->status,
            'status_after' => $order->status,
            'note' => 'Cơ cấu thực đóng size '.$mainSizeLabel.': '.$allocationInput->map(
            ...
            if ($item) {
                ...
                if ($request->boolean('packed_quantity_only')) {
                    if (! isset($validated['item_packed_quantity'])) {
                        throw ValidationException::withMessages(['item_packed_quantity' => 'Vui lòng nhập số lượng đóng thực tế.']);
                    }
                    ...
                    if (! $isCutProduct && ! $order->allowsWarehouseQuantityChange((int) $item->product_id)) {
                        throw ValidationException::withMessages(['item_packed_quantity' => 'Sale chưa cho phép kho cập nhật số lượng đóng thực tế của sản phẩm này.']);
                    }
                ...
                if ($request->boolean('clear_item_weight')) {
                    ...
                    app(WarehousePackedOrderService::class)->recalculate($order);
                ...
                if (isset($validated['item_packed_quantity'])) {
                    if (! $isCutProduct && ! $order->allowsWarehouseQuantityChange((int) $item->product_id)) {
                        throw ValidationException::withMessages(['item_packed_quantity' => 'Sale chưa cho phép cập nhật số lượng đóng thực tế.']);
                    }
                    $requiredWeight = round((float) $item->quantity * (float) $item->effective_unit_weight, 3);
                    if ($newWeight < $requiredWeight - 0.000001) {
                        throw ValidationException::withMessages(['item_actual_weight' => 'Hàng pha lóc phải đủ '.$requiredWeight.' kg theo bill khi cập nhật số lượng đóng thực tế.']);
                    }
                ...
                if (! $isCutProduct && $itemSize > 0 && $packedQuantityForWeight > 0) {
                    ...
                    if ($averageWeight < $averageMin - 0.000001 || $averageWeight > $averageMax + 0.000001) {
                        $message = 'Khối lượng bình quân phải nằm trong khoảng '
                            .rtrim(rtrim(number_format($averageMin, 2, '.', ''), '0'), '.')
                            .'–'.rtrim(rtrim(number_format($averageMax, 2, '.', ''), '0'), '.')
                            .' kg/sản phẩm.';
                        if ($expectsJson) {
                            return response()->json(['ok' => false, 'message' => $message], 422);
                        }
    ...
    private function refreshChangedItemReservations(Order $order, $item): void
    {
        ...
        if ($remaining > 0) {
            throw ValidationException::withMessages(['items' => 'Không đủ tồn kho cho số lượng mới. Vui lòng bổ sung hàng trước khi tăng số lượng.']);
        }
    }
    ...
    public function requestAdjustment(Request $request, Order $order)
    {
        ...
        if ($order->warehouse_adjustment_status === Order::WAREHOUSE_ADJUSTMENT_STATUS_PENDING_SALE_CONFIRMATION) {
            throw ValidationException::withMessages(['items' => 'Đơn đang chờ Sale xác nhận điều chỉnh trước đó.']);
        }
        ...
        if ($order->status === Order::STATUS_PACKING && ! $canDirectAdjust) {
            throw ValidationException::withMessages(['items' => 'Sale chưa cho phép kho đổi số lượng sản phẩm này khi đang đóng hàng.']);
        }
        if ($canDirectAdjust) {
            DB::transaction(function () use ($order, $changes, $validated): void {
                ...
                if (! in_array($order->status, [Order::STATUS_APPROVED, Order::STATUS_READY_TO_PACK, Order::STATUS_PACKING], true)
                    ...
                    throw ValidationException::withMessages(['items' => 'Trạng thái đơn đã thay đổi. Vui lòng tải lại trang.']);
                }
                foreach ($changes as $change) {
                    ...
                    if (! $order->allowsWarehouseQuantityChange((int) $productId)
                        ...
                        throw ValidationException::withMessages(['items' => 'Quyền hoặc số lượng đã thay đổi. Vui lòng tải lại trang.']);
                    }
                }
    ...
    public function returns(Request $request)
    {
        ...
        if ($orders->isNotEmpty()) {
            $orderReturnCreatedMap = OrderReturn::query()
    ...
    public function stockIn(Request $request)
    {
        ...
        $maxEdits = (int) (Setting::get('stock_in_max_edits', 3));
    ...
    public function confirmIncomingInventoryTransfer(WarehouseInventoryTransfer $transfer)
    {
        ...
        if ($transfer->dispatchEntry?->slip?->status === WarehouseDispatchSlip::STATUS_DRAFT) {
                ...
                foreach ($validated['items'] as $row) {
                    ...
                    if ($delta !== 0) {
                        ...
                        if ($delta < 0) {
                            ...
                            $completedQty = (int) OrderItem::query()
